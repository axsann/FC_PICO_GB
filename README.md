...
#### カートリッジ RAM セーブ（ゲーム内セーブ）

ゲーム内でセーブした後、**SELECT + START 同時押し**でフラッシュに保存します。
...
ゲームの状態をいつでも保存・復元できます。

| 操作 | 機能 |
|------|------|
| **SELECT + A** | セーブステート保存 |
| **SELECT + B** | セーブステート読み込み |

**表示メッセージ:**

| メッセージ | 意味 |
|-----------|------|
| STATE SAVED | セーブステート保存成功 |
| STATE LOADED | セーブステート読み込み成功 |
| NO DATA | セーブステートが存在しない |
| NO FS | ファイルシステムが無効（Flash Size 設定を確認） |
| ERROR | その他のエラー（ROM 不一致など） |

- セーブステートは `/saves/TITLE.state` に保存されます（約17KB）
- 1ゲームにつき1スロットです
